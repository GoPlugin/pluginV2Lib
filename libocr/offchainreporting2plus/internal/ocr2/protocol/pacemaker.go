...
import (
	...
	"github.com/Goplugin/PluginV2Lib/libocr/offchainreporting2plus/internal/ocr2/protocol/persist"
...
func (pace *pacemakerState) restoreNeFromTransmitter() {
	...
	ok := pace.subprocesses.BlockForAtMost(
		...
		func(ctx context.Context) {
			configDigest, epoch, err = pace.contractTransmitter.LatestConfigDigestAndEpoch(ctx)
		},
	)

	if !ok {
		pace.logger.Error("Pacemaker: latestConfigDigestAndEpoch timed out while restoring ne", commontypes.LogFields{
	...
	if err != nil {
		pace.logger.Error("Pacemaker: latestConfigDigestAndEpoch returned error while restoring ne", commontypes.LogFields{
			"error": err,
		})
		return
	}
